...
class AncientGreekLemmatizer(Lemmatizer):
    ...
    def __init__(self, lookups, *args, **kwargs):
        ...
        super(AncientGreekLemmatizer, self).__init__(lookups)
        self.lemma_lookups = {}
        for tag in [
            "ADJ",
            "ADP",
   ...
   ### THis is taken from the German lemmatizer and will not work for Classical Greek.
   ## We should look however into this function and assest if we need something similar for grc;
   ## We may not need it however because we are using a lookup table with the complete declension of the adj.
